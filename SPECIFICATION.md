...
**기술 구현**:
- HTML5 Drag & Drop API 또는 react-dnd
- 드래그 시작: 카드 강조 효과
- 드롭 가능 영역: 호버 시 하이라이트
- 정답 확인: 드롭된 부서와 정답 비교
...
**기술 구현**:
- 실시간 입력 감지 (onChange)
- 복사/붙여넣기 방지: `onPaste`, `onCopy`, `onCut` 이벤트 차단
- 게이지 바 애니메이션:
  - 입력 길이에 비례하여 진행률 계산
...
### 4.4 유형 4: OX 문제 (캐릭터 힌트)

**설명**: 문장의 참/거짓 판단, 마우스 위치에 따라 캐릭터가 힌트 제공
...
**인터랙션**:
- 마우스가 **정답에 가까워지면**: 캐릭터 😊 (웃는 표정)
- 마우스가 **오답에 가까워지면**: 캐릭터 😣 (찡그린 표정)
- 클릭 전까지는 힌트 제공

**기술 구현**:
- `onMouseMove` 이벤트로 마우스 위치 추적
- 거리 계산 (마우스 ↔ O/X 버튼)
- 캐릭터 이미지/이모지 동적 변경
