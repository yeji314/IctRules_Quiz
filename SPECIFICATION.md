...
### 3.4 LuckyDraw 문제 출제 및 추첨 규칙

#### 3.4.1 LuckyDraw 문제 노출 조건
- **말풍선 문구**: LuckyDraw 문제 출제 시 좌하단 캐릭터 말풍선에 다음 문구 표시
  - "이번문제는 luckydraw문제입니다. 맞추면 선물 획득 기회가 있어요!"
  - 일반 문제일 때는 이 문구가 절대 표시되지 않음

- **LuckyDraw 등장 조건**:
  - 현재 세션에서 **정답을 3개 이상 맞춘 경우** LuckyDraw 문제 출현 가능
  - 직전 문제가 LuckyDraw가 아니어야 함 (연속 LuckyDraw 방지)
  - **해당 월(이벤트)에 이미 선물에 당첨된 사용자에게는 절대 출제 안 됨**
...
#### 3.4.2 LuckyDraw 추첨 규칙
- **추첨 발생 조건**:
  - LuckyDraw 문제를 **첫 시도에 정답**을 맞춰야 추첨 진행
  - 오답을 한 번이라도 낸 경우 추첨 없음 (일반 문제와 동일하게 처리)

- **추첨 제외 조건**:
  - 이미 해당 월(이벤트)에서 선물에 당첨된 사용자
  - 최대 당첨자 수 도달
  - LuckyDraw 문제를 틀린 경우
...
#### 3.4.3 결과 페이지 표시 규칙
- **선물 획득 여부**: 반드시 "이번 회차(세션)" 기준으로만 표시
  - ✅ 올바른 예: 1회차에 당첨, 2회차에 미당첨 → 2회차 결과: "선물 획득 없음"
  - ❌ 잘못된 예: 1회차에 당첨, 2회차에 미당첨 → 2회차 결과: "선물 획득함" (누적 X)
- 결과 페이지는 `session.won_prize_this_session` 값만 사용

#### 3.4.4 출제 알고리즘
```
1. 현재 세션에서 맞춘 문제 수 확인
2. 해당 월에 이미 선물 당첨 여부 확인 (hasWonPrizeThisMonth)
3. 당첨된 적 있으면 LuckyDraw 문제 절대 출제 안 함
4. 맞춘 문제 수 >= 3 && 직전 문제가 LuckyDraw 아님 → LuckyDraw 풀 포함
5. 남은 문제에서 가중치 랜덤 선택 (LuckyDraw 40%)
```
